...
function registerAlpineDirective(name, callback) {
...
function createFastUI(userConfig) {
  ...
  async function init() {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("fastui:ready", { detail: { version: "0.1.6" } }));
    } catch (error) {
      console.error("[FastUI] Initialization error:", error);
      throw error;
    }
  }
  function setupAlpineInterceptor() {
    if (isAlpineLoaded()) {
      flushDirectives();
      return;
    }
    ...
    setTimeout(() => clearInterval(checkAlpine), 5e3);
  }
  ...
  function reinit(el) {
    if (isAlpineLoaded() && window.Alpine.initTree) {
