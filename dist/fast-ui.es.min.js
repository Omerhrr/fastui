...
async function loadAllDependencies() {
  ...
  await Promise.all([
    loadECharts().catch(() => debugLog()),
    loadFlowbite().catch(() => debugLog())
  ]);
  await loadAlpine();
  debugLog();
}
 ...
 * @license MIT
 */
const pendingDirectives = /* @__PURE__ */ new Map();
function preRegisterDirective(name, callback) {
...
function createFastUI(userConfig) {
  ...
  async function init() {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("fastui:ready", { detail: { version: "0.1.4" } }));
    } catch (error) {
      console.error("[FastUI] Initialization error:", error);
      throw error;
    }
  }
  async function waitForAlpine(timeout = 5e3) {
  ...
  function registerPendingDirectives() {
    if (!hasAlpine()) return;
